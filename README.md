...
* Right now all the data wueries and calculations are done in memory using a JSON query library called ObjectPath.  This was due to the fact that the only data I had access to was via the NHL API instead of a proper database.  If I had a database to query, that would allow me to push most calculations and queries to the db-layer and prevent so much data from being pulled into memory.